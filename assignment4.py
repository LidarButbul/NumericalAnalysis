...
class Assignment4A:
    ...
    def fit(self, f: callable, a: float, b: float, d:int, maxtime: float) -> callable:
        ...
        n = int((b-a)/(maxtime / 650000))
        data_point = np.linspace(a, b, n)
        ...
        M_inverse = linalg.inv(M)  #M^-1
        T_transpose = np.transpose(T)  #T^T
        T_transpose_T_times = np.matmul(T_transpose, T)
        T_transpose_T_times_inv = linalg.inv(T_transpose_T_times)  #(T^T*T)^-1
        C1 = np.matmul(M_inverse, T_transpose_T_times_inv)
        C2 = np.matmul(C1, T_transpose)
        C= np.matmul(C2, Y)

        def result(x):
            t = (x - a)/(b - a)
            Tmetrix = np.array([t ** 3, t ** 2, t, 1], dtype= np.float32)
            T_M = np.matmul(Tmetrix, M)
            y = np.matmul(T_M,C)
            return float(y)
        return result
